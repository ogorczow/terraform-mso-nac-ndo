locals {
  ...
  template_policies = [for template_policy in try(local.ndo.template_policies, []) : template_policy if var.manage_template_policies && (length(var.managed_template_policies) == 0 || contains(var.managed_template_policies, template_policy.name))]
